...
### Deploy CLIs (Cloud Desktop)
- `gh` (GitHub CLI) is preinstalled at `/exec-daemon/gh` and authenticated as the platform `cursor` account (scoped token). For deploys/PRs under your own identity, run `gh auth login` or export `GH_TOKEN` in the Cloud Desktop terminal.
- `vercel` (Vercel CLI, pinned to `56.3.1` to match `build:vercel`) is installed to `$HOME/.npm-global` and added to `PATH` via `~/.bashrc`. The startup update script also reinstalls it (`npm install -g vercel@56.3.1 --prefix "$HOME/.npm-global"`) so it survives even if the snapshot does not capture it. If `vercel` is ever not found, run that same command or open a new shell so `~/.bashrc` re-exports `PATH`.
- Vercel auth: a personal `VERCEL_TOKEN` secret is configured and injected as an env var. Use it non-interactively, e.g. `vercel --token "$VERCEL_TOKEN" whoami` / `vercel --token "$VERCEL_TOKEN" link` / `... deploy`. (`VERCEL_OIDC_TOKEN` is also injected for build-time OIDC.) The `mekor` project is not linked locally yet — run `vercel link --token "$VERCEL_TOKEN"` once before deploying.
