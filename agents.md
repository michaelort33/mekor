...
- Vercel auth is not preconfigured: `VERCEL_OIDC_TOKEN` (build-time OIDC) is injected but the CLI needs a personal token for `vercel deploy`. Either run `vercel login` interactively in the Cloud Desktop, or add a `VERCEL_TOKEN` secret and use `vercel --token "$VERCEL_TOKEN" ...`.
