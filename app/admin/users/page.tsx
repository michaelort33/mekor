...
const ROLE_OPTIONS: AdminUser["role"][] = ["visitor", "member", "admin"];
  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        <div>
          <h1>Manage Users</h1>
          <p>Promote visitors and control profile visibility.</p>
        </div>
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id}>
                  ...
                  <td>
                    <button type="button" onClick={() => saveUser(user)} disabled={savingId === user.id}>
